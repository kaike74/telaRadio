        ...
        if (data.success) {
            ...
            if (data.insercoesRecentes && Array.isArray(data.insercoesRecentes)) {
                if (data.insercoesRecentes.length > 0) {
                    LoggerOtimizado.log(`Ticker: ${data.insercoesRecentes.length} inserções`, 'ticker');
                    atualizarTicker({
                        insercoesRecentes: data.insercoesRecentes
                    });
                }
            }
            ...
            // ✨ NOVO: Buscar e exibir logs de inserções
            buscarLogsInsercoes();
        } else {
            LoggerOtimizado.erro(`Resposta com success=false: ${data.error}`);
        }

    } catch (error) {
        LoggerOtimizado.erro(`Erro ao buscar inserções recentes`, error);
    }
}

/**
 * ✨ NOVO: Buscar logs de inserções do backend e exibir no console (F12)
 * Formato: HORARIO RECEBIDO - EMISSORA - CAMPANHA - HORARIO QUE IRA SER EXIBIDO
 */
async function buscarLogsInsercoes() {
    try {
        const response = await fetch(`${CONFIG.API_BASE}/api/logs/insercoes`);
        ...
        if (data.sucesso && data.logs) {
            const logs = data.logs;
            const agora = new Date();
            const umaHoraAtras = new Date(agora.getTime() - 60 * 60 * 1000); // 1 hora atrás
            
            // 🟢 INSERÇÕES SENDO EXIBIDAS (últimas 1 hora)
            if (logs.exibidas && logs.exibidas.length > 0) {
                // Filtrar por inserções da última 1 hora
                const exibidasUltimaHora = logs.exibidas.filter(log => {
                    const [hora, minuto] = log.horaReal.split(':').map(Number);
                    const horaAtual = agora.getHours();
                    const minutoAtual = agora.getMinutes();
                    
                    // Converter para minutos desde 00:00
                    const tempoLog = hora * 60 + minuto;
                    const tempoUmaHora = umaHoraAtras.getHours() * 60 + umaHoraAtras.getMinutes();
                    const tempoAgora = horaAtual * 60 + minutoAtual;
                    
                    // Se cruzou meia-noite, ajustar lógica
                    if (tempoAgora >= tempoUmaHora) {
                        return tempoLog >= tempoUmaHora && tempoLog <= tempoAgora;
                    } else {
                        return tempoLog >= tempoUmaHora || tempoLog <= tempoAgora;
                    }
                }).slice(-20); // Últimas 20 inserções da última hora
                
                if (exibidasUltimaHora.length > 0) {
                    console.log(`%c${'═'.repeat(100)}`, 'color: #E03D99; font-weight: bold; font-size: 12px;');
                    console.log(`%c✅ INSERÇÕES SENDO EXIBIDAS - ÚLTIMA 1 HORA (${exibidasUltimaHora.length})`, 'color: #E03D99; font-weight: bold; font-size: 12px;');
                    console.log(`%c${'═'.repeat(100)}`, 'color: #E03D99; font-weight: bold; font-size: 12px;');
                    
                    exibidasUltimaHora.forEach((log, idx) => {
                        console.log(
                            `%c${(idx + 1).toString().padStart(3)}. %c${log.horaReal.padEnd(10)} | %c${log.emissora.padEnd(45)} | %c${log.proposta.padEnd(35)} | %c→ ${log.horaExibicao}`,
                            'color: #7B2CBF; font-weight: bold;',
                            'color: #5A5FFF;',
                            'color: #E03D99;',
                            'color: #00D9FF;',
                            'color: #90EE90;'
            ...
            // 🟡 INSERÇÕES FILTRADAS (últimas 10 que serão exibidas em breve)
            if (logs.filtradas && logs.filtradas.length > 0) {
                // Pegar as 10 mais recentes que serão exibidas em breve
                const filtradas10Proximas = logs.filtradas.slice(-10).reverse(); // Reverter para mostrar as próximas primeiro
                
                console.log(`%c${'─'.repeat(100)}`, 'color: #7B7B7B; font-size: 10px;');
                console.log(`%c⏸️ PRÓXIMAS INSERÇÕES A EXIBIR - FILTRADAS PELO DELAY DE 2H (${filtradas10Proximas.length} de ${logs.filtradas.length})`, 'color: #FFD700; font-weight: bold; font-size: 11px;');
                console.log(`%c${'─'.repeat(100)}`, 'color: #7B7B7B; font-size: 10px;');
                
                filtradas10Proximas.forEach((log, idx) => {
                    // Extrair horário de exibição
                    let horaExibicao = log.horaExibicao;
                    if (log.horaExibicao.includes('[FILTRADO]')) {
                        horaExibicao = log.horaExibicao.substring(10).trim();
                    }
                    
                    console.log(
                        `%c${(idx + 1).toString().padStart(3)}. %c${log.horaReal.padEnd(10)} | %c${log.emissora.padEnd(45)} | %c${log.proposta.padEnd(35)} | %c⏳ ${horaExibicao}`,
                        'color: #FFD700;',
                        'color: #A9A9A9;',
                        'color: #FFD700;',
                        'color: #C0C0C0;',
